      ...
      .replace(VAR_RULE_REGEX, (match, prefix, colorType, hue, opacity, contrast, suffix) => {
        ...
        if (colorType === 'foreground') {
          if (hue == 'shadow') {
            ...
            return 'RGB(var(--foreground-shadow))';
          } else {
            if (['1', '2', '3', '4'].indexOf(hue) === -1) {
              hue = '1';
            }
            // return `${prefix} color($foreground, "${hue}") ${suffix}`;
            return `${prefix} RGB(var(--foreground-${hue})) ${suffix}`;
          }
        }
        ...
        if (opacity) {
          ...
          return `${prefix} RGBA(var(--${colorType}-${hue}), ${opacity}) ${suffix}`;
        } else {
          // return `${prefix} ${colorFunction}(${params.join(', ')}) ${suffix}`;
          return `${prefix} RGB(var(--${colorType}-${hue})) ${suffix}`;
        }
      });
  }
}
