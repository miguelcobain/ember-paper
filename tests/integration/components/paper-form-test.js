...
module('Integration | Component | paper form', function (hooks) {
  ...
  skip('works without using contextual components', async function (assert) {
    assert.expect(4);
  ...
  test('works without using contextual components by binding in the yielded parent', async function (assert) {
    assert.expect(4);

    await render(hbs`
      {{#paper-form as |form|}}
        {{paper-input parentComponent=form.parent value=foo onChange=(action (mut foo)) label="Foo"}}
        {{paper-input parentComponent=form.parent value=bar onChange=(action (mut bar)) label="Bar" errors=errors}}
  ...
  test('form submit button component can be customized by passing `submitButtonComponent`', async function (assert) {
    ...
    this.owner.register(
      'component:custom-submit-button',
      Component.extend({
