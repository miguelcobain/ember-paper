...
function buildGridModel(tileTmpl) {
  ...
  for (let j = 0; j < 11; j++) {
    ...
    // Use assign if available. TODO: Remove if/when ember-paper requires ember-2.5.0 or higher.
    let it = (Ember.assign || Ember.merge)({}, tileTmpl);
    it.icon = it.icon + (j + 1);
...
export default Ember.Controller.extend({
  ...
  calculateColorTiles() {
    let tiles = [];
    for (let i = 0; i < 46; i++) {
      tiles.push(Ember.Object.create({
        style: randomColor(),
        colspan: randomSpan(),
        rowspan: randomSpan()
      }));
  ...
  colorTiles: Ember.computed(function() {
    return Ember.A([]);
  })
});
